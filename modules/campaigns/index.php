...
if ($get_id) {
	...
	$GET_PAGEINFO_q = mysql_query("SELECT L.*,T.`access`,T.`template`,T.`slide` FROM `mod_campaigns` L,`mod_campaigns_list` T WHERE T.`id`=L.`page_id` AND L.`page_id` = $get_id AND L.`lang_id`=".LANG) or die(mysql_error());
	...
	if ($GET_PAGEINFO_c > 0) {
		...
		if (!do_no_access($GET_PAGEINFO_r["access"]))
		{
			...
			$xsd =  "<a href = \"./,campaigns,print_{$get_id}\"><% LANG_PRINTABLE %></a>";
			function PAGE_TEXT() { 
